...
{
  uint32_t __regControl         __asm__("control");
  return(__regControl);
}
...
{
  uint32_t __regIPSR          __asm__("ipsr");
  return(__regIPSR);
}
#endif /*  __ARMCC_VERSION  */
...
{
  uint32_t __regAPSR          __asm__("apsr");
  return(__regAPSR);
}
#endif /*  __ARMCC_VERSION  */
...
{
  uint32_t __regXPSR          __asm__("xpsr");
  return(__regXPSR);
}
#endif /*  __ARMCC_VERSION  */
...
{
  uint32_t __regProcessStackPointer  __asm__("psp");
...
{
  uint32_t __regMainStackPointer     __asm__("msp");
...
{
  uint32_t __regPriMask         __asm__("primask");
  return(__regPriMask);
}
#endif /*  __ARMCC_VERSION  */
...
{
  uint32_t __regBasePri         __asm__("basepri");
  return(__regBasePri);
}
#endif /*  __ARMCC_VERSION  */
...
{
  uint32_t __regFaultMask       __asm__("faultmask");
...
{
#if (__FPU_PRESENT == 1)
  uint32_t __regfpscr         __asm__("fpscr");
  return(__regfpscr);
#else
   return(0);
#endif
...
{
  __asm__("mrs r0, psr");           // assembler does not know "xpsr"
}
...
{
  __asm__ volatile ("MSR faultmask, %0" : : "r" (faultMask) );
}
...
__attribute__( ( always_inline ) ) static inline uint32_t __get_FPSCR(void)
{
  ...
  __asm__ volatile ("MRS %0, fpscr" : "=r" (result) );
  return(result);
#else
   return(0);
#endif
...
{
#if (__FPU_PRESENT == 1)
  __asm__ volatile ("MSR fpscr, %0" : : "r" (fpscr) );
#endif
}
